...
const HOST = process.env.HOST || (process.env.BODHIBEATS_PLATFORM === 'android' ? '127.0.0.1' : '0.0.0.0');
const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const COOKIE_FILE = process.env.COOKIE_FILE || path.join(__dirname, '.cookie');
const QQ_COOKIE_FILE = process.env.QQ_COOKIE_FILE || path.join(__dirname, '.qq-cookie');
const UPDATE_WORK_DIR = process.env.BODHIBEATS_UPDATE_DIR || path.join(__dirname, 'updates');
const UPDATE_DOWNLOAD_DIR = process.env.BODHIBEATS_UPDATE_DOWNLOAD_DIR || path.join(UPDATE_WORK_DIR, 'downloads');
const UPDATE_PATCH_BACKUP_DIR = process.env.BODHIBEATS_PATCH_BACKUP_DIR || path.join(UPDATE_WORK_DIR, 'backups', 'patches');
const BEATMAP_CACHE_DIR = process.env.BODHIBEATS_BEAT_CACHE_DIR || path.join(__dirname, 'cache', 'beatmaps');
const APP_PACKAGE = readPackageInfo();
const APP_VERSION = process.env.BODHIBEATS_VERSION || APP_PACKAGE.version || '0.9.11';
const UPDATE_CONFIG = readUpdateConfig(APP_PACKAGE);
const BODHIBEATS_PLATFORM = process.env.BODHIBEATS_PLATFORM || 'desktop';
const PATCH_ENABLED = BODHIBEATS_PLATFORM === 'desktop';
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.bodhibeats && pkg.bodhibeats.update) || {};
  const repoHint = process.env.BODHIBEATS_UPDATE_REPOSITORY
  ...
  const owner = process.env.BODHIBEATS_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.BODHIBEATS_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.BODHIBEATS_UPDATE_MANIFEST
      || process.env.BODHIBEATS_UPDATE_MANIFEST_URL
      || process.env.BODHIBEATS_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.BODHIBEATS_UPDATE_MIRRORS || process.env.BODHIBEATS_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `BodhiBeats-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `BodhiBeats-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('BodhiBeats v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `BodhiBeats/${APP_VERSION}` },
    });
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `BodhiBeats-${latestVersion}-Setup.exe`;
  ...
  return {
    ...
    release: {
      ...
      name: 'BodhiBeats v' + latestVersion,
      version: latestVersion,
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      ...
      headers: {
        'User-Agent': `BodhiBeats/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('BodhiBeats v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `BodhiBeats-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `BodhiBeats-${version || APP_VERSION}.exe`;
}
...
function writePatchFile(job, file) {
  ...
  const tmp = target + '.bodhibeats-patch';
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'bodhibeats-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'bodhibeats',
      productName: APP_PACKAGE.productName || 'BodhiBeats',
      version: APP_VERSION,
      update: {
  ...
  if (pn === '/api/update/patch') {
    if (!PATCH_ENABLED) {
      sendJSON(res, { ok: false, error: 'PATCH_UNSUPPORTED_ON_PLATFORM' }, 403);
      return;
    }
    try {
      const info = await fetchLatestUpdateInfo();
