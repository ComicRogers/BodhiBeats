// BodhiBeats Capacitor Bridge — 由 Android WebView 在页面加载前注入
// 提供 desktopWindow 兼容层 + InAppBrowser 登录桥接 + 文件导入导出
(function(){
  if (window.__bodhibeatsCapacitorBridge) return;
  window.__bodhibeatsCapacitorBridge = true;
  ...
  window.desktopWindow = {
    ...
    openUpdateInstaller: function(filePath){
      if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Browser) {
        window.Capacitor.Plugins.Browser.open({ url: 'https://github.com/XxHuberrr/BodhiBeats/releases' });
      }
  ...
  function exportJson(payload){
    ...
    if (!Filesystem) {
      ...
      a.href = url; a.download = payload.defaultName || 'bodhibeats-export.json'; a.click();
      setTimeout(function(){ URL.revokeObjectURL(url); }, 1000);
      return Promise.resolve({ ok: true });
    }
    var fileName = (payload.defaultName || 'bodhibeats-export.json').replace(/[\\/:*?"<>|]+/g, '-');
    var text = typeof payload.text === 'string' ? payload.text : JSON.stringify(payload.data || {}, null, 2);
    return Filesystem.writeFile({
      path: 'BodhiBeats/' + fileName,
