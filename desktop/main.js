...
const APP_NAME = 'BodhiBeats';
const APP_USER_MODEL_ID = 'com.bodhibeats.desktop';
const APP_ICON_ICO = path.join(__dirname, '..', 'build', 'icon.ico');
const NETEASE_LOGIN_PARTITION = 'persist:bodhibeats-netease-login';
const NETEASE_LOGIN_URL = 'https://music.163.com/#/login';
const QQ_LOGIN_PARTITION = 'persist:bodhibeats-qqmusic-login';
...
function sendGlobalHotkeyAction(action) {
  ...
  mainWindow.webContents.send('bodhibeats-global-hotkey', { action });
}

function unregisterBodhiBeatsGlobalHotkeys() {
...
function configureBodhiBeatsGlobalHotkeys(bindings = []) {
  unregisterBodhiBeatsGlobalHotkeys();
  const results = [];
  const seen = new Set();
...
function shouldEnsureDesktopShortcut() {
  ...
  if (process.env.BodhiBeats_NO_DESKTOP_SHORTCUT === '1') return false;
  return app.isPackaged || process.env.BodhiBeats_CREATE_DESKTOP_SHORTCUT === '1';
}
...
function ensureDesktopShortcut() {
  ...
  try {
    ...
    const shortcut = {
      ...
      args: '',
      description: 'BodhiBeats desktop music player',
...
public class BodhiBeatsMousePoll {
  [DllImport("user32.dll")] public static extern short GetAsyncKeyState(int vKey);
}
"@
$prev = $false
while ($true) {
  $down = (([bodhibeatsMousePoll]::GetAsyncKeyState(4) -band 0x8000) -ne 0)
...
function broadcastDesktopLyricsLockState() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('bodhibeats-desktop-lyrics-lock-state', { locked });
  }
...
function broadcastDesktopLyricsEnabledState(enabled) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('bodhibeats-desktop-lyrics-enabled-state', { enabled: !!enabled });
  }
}
...
function sendDesktopLyricsState() {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) return;
  desktopLyricsWindow.webContents.send('bodhibeats-desktop-lyrics-state', desktopLyricsState);
}
...
function createDesktopLyricsWindow(payload = {}) {
  ...
  desktopLyricsWindow = new BrowserWindow({
    ...
    skipTaskbar: true,
    show: false,
    title: 'BodhiBeats Desktop Lyrics',
    webPreferences: {
...
if (-not ("BodhiBeatsNativeWin" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public class BodhiBeatsNativeWin {
...
$progman = [bodhibeatsNativeWin]::FindWindow("Progman", $null)
$result = [IntPtr]::Zero
[bodhibeatsNativeWin]::SendMessageTimeout($progman, 0x052C, [IntPtr]::Zero, [IntPtr]::Zero, 0, 1000, [ref]$result) | Out-Null
$script:workerw = [IntPtr]::Zero
$enum = [bodhibeatsNativeWin+EnumWindowsProc]{
  param([IntPtr]$top, [IntPtr]$param)
  $shell = [bodhibeatsNativeWin]::FindWindowEx($top, [IntPtr]::Zero, "SHELLDLL_DefView", $null)
  if ($shell -ne [IntPtr]::Zero) {
    $script:workerw = [bodhibeatsNativeWin]::FindWindowEx([IntPtr]::Zero, $top, "WorkerW", $null)
  }
  return $true
}
[bodhibeatsNativeWin]::EnumWindows($enum, [IntPtr]::Zero) | Out-Null
if ($script:workerw -eq [IntPtr]::Zero) { $script:workerw = $progman }
$target = [IntPtr]::new([Int64]${hwnd})
[bodhibeatsNativeWin]::SetParent($target, $script:workerw) | Out-Null
[bodhibeatsNativeWin]::SetWindowPos($target, [IntPtr]::Zero, 0, 0, 0, 0, 0x0013) | Out-Null
`;
...
function sendWallpaperState() {
  if (!wallpaperWindow || wallpaperWindow.isDestroyed()) return;
  wallpaperWindow.webContents.send('bodhibeats-wallpaper-state', wallpaperState);
}
...
ipcMain.handle('bodhibeats-hotkeys-configure-global', (_event, bindings) => {
  return configureBodhiBeatsGlobalHotkeys(bindings);
});

ipcMain.handle('bodhibeats-export-json-file', async (event, payload = {}) => {
  try {
    const owner = getSenderWindow(event);
    const defaultName = String(payload.defaultName || 'bodhibeats-export.json').replace(/[\\/:*?"<>|]+/g, '-');
    const result = await dialog.showSaveDialog(owner, {
      title: '导出 BodhiBeats 存档',
...
ipcMain.handle('bodhibeats-import-json-file', async (event) => {
  try {
    const owner = getSenderWindow(event);
    const result = await dialog.showOpenDialog(owner, {
      title: '导入 BodhiBeats 存档',
      properties: ['openFile'],
      filters: [{ name: 'JSON', extensions: ['json'] }],
    });
...
ipcMain.handle('bodhibeats-open-update-installer', async (_event, filePath) => {
  try {
...
ipcMain.handle('bodhibeats-restart-app', async () => {
  try {
    app.relaunch();
    app.exit(0);
    return { ok: true };
...
ipcMain.handle('bodhibeats-desktop-lyrics-set-enabled', async (_event, enabled, payload) => {
  try {
    if (enabled) {
...
ipcMain.handle('bodhibeats-desktop-lyrics-update', async (_event, payload) => {
  try {
...
ipcMain.handle('bodhibeats-desktop-lyrics-set-dragging', async () => {
  return { ok: true };
});

ipcMain.handle('bodhibeats-desktop-lyrics-set-pointer-capture', async (_event, active) => {
  try {
...
ipcMain.handle('bodhibeats-desktop-lyrics-set-hot-bounds', async (_event, bounds) => {
  try {
...
ipcMain.handle('bodhibeats-desktop-lyrics-set-lock-state', async (_event, locked) => {
  try {
...
ipcMain.handle('bodhibeats-desktop-lyrics-move-by', async (_event, dx, dy) => {
  try {
...
ipcMain.handle('bodhibeats-wallpaper-set-enabled', async (_event, enabled, payload) => {
  try {
...
ipcMain.handle('bodhibeats-wallpaper-update', async (_event, payload) => {
  try {
...
async function createWindow() {
  ...
  process.env.BodhiBeats_UPDATE_DIR = getUpdateDownloadDir();
  try {
