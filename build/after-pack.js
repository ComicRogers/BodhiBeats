...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for BodhiBeats icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'BodhiBeats';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`BodhiBeats executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`BodhiBeats icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting BodhiBeats resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'BodhiBeats',
    '--set-version-string', 'ProductName', 'BodhiBeats',
    '--set-version-string', 'CompanyName', 'BodhiBeats',
